...
export class Heap {
  ...
  protected __isMax: boolean = false;
  protected __compareFunc: CompareCallback;


  constructor(n: number, compare: CompareCallback) {
    this.__heap = new Array(n + 2);
    this.__size = n;
    this.__head = 0;
    this.__last = 0;
    this.__compareFunc = compare;
  }

  protected swap(a: number, b: number){
    const _ = this.__heap[a];
    this.__heap[a] = this.__heap[b];
    this.__heap[b] = _;
  }
  ...
  protected heapify() {
    ...
    while(true) {
      ...
      if(
        index * 2 < n 
        && this.__compareFunc(this.__heap[maxPos], this.__heap[index * 2])
      ) {
        maxPos = index * 2;
      }
      ...
      if(
        index * 2 + 1 < n 
        && this.__compareFunc(this.__heap[maxPos], this.__heap[index * 2 + 1])
      ) {
        maxPos = index * 2 + 1;
      }
